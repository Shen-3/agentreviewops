...
from agentreview.models import AgentReviewConfig, DiffFile, RiskAnalysis
...
def analyze_diff_text(diff_text: str, *, config: AgentReviewConfig) -> AnalysisExecutionResult:
    ...
    markdown = generate_markdown_report(analysis, changed_files, config=config, ai_summary=ai_summary)
    return AnalysisExecutionResult(changed_files=changed_files, analysis=analysis, markdown=markdown)
