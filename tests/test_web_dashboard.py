...
def test_dashboard_contains_empty_loading_error_states() -> None:
    ...
    assert "Loading analysis runs" in app_tsx
    assert "Unable to load analysis runs" in app_tsx
    assert "No analysis runs" in app_tsx
    assert "/api/analysis-runs" in app_tsx
    assert "API unavailable. Showing demo analysis data." in app_tsx
